...
image_of_bishoy = face_recognition.load_image_file("D:\image.folder\Bishoy Bassem.jpg")
image_of_bishoy2 = BytesIO(Image.open("D:\image.folder\Bishoy Bassem.jpg").tobytes())
...
def trigger_capture():
    ...
    esp_cam_url = "http://192.168.51.43/capture"
    try:
        ...
        if response.status_code == 200:
           ...
           return jsonify({"error": f"Failed to capture image. Status Code: {response.status_code}", "content": response.content.decode()}), 500
...
def play_sound_on_esp(file_number):
    ...
    esp_play_sound_url = "http://192.168.51.11" + str(file_number)
    try:
